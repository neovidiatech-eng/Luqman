...
//     if (text.includes("رفض")) return "rejection";
//     if (text.includes("قبول") || text.includes("الموافقة")) return "approval";
...
//                   <ChevronLeft
//                     size={20}
//                     className="text-[var(--border)] group-hover:text-[var(--secondary)] transition-colors"
//                   />
//                 </div>
//               </div>
//             </div>
...
export default function Notifications() {
  ...
  const { mutate: markAllRead, isPending: markingAll } =
    useMarkAllNotificationsRead();
  const { mutate: deleteOne, isPending: deletingOne } = useDeleteNotification(); // ← جديد
